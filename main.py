...
class Consulta(tk.Tk):
    def __init__(self):
        ...
        self.atualiza_btn=ttk.Button(self,text='Atualizar Lista',command=self.atualizar)
        self.atualiza_btn.grid(row=1,column=1)

        self.inserir_btn=ttk.Button(self,text='Inserir Registro',command=self.inserir)
        self.inserir_btn.grid(row=3,column=0)
...
class InfoDisplay(ttk.Frame):
    ...
    def atualizar(self):
        self.atualiza_window=Atualizar(self.id)
        self.atualiza_window.mainloop()
